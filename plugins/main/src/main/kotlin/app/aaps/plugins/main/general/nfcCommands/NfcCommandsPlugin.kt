...
constructor(
    ...
    private val isBackgroundDetectionEnabled: Boolean
        get() = preferences.get(BooleanKey.NfcEnableBackgroundDetection)

    init {
        appScope.launch {
            preferences.observe(BooleanKey.NfcEnableBackgroundDetection).collectLatest {
    ...
    override fun onStateChange(type: PluginType?, oldState: State?, newState: State?) {
        ...
        if (type == PluginType.GENERAL) {
            updateComponentState()
        }
    }

    private fun updateComponentState() {
        val backgroundEnabled = isBackgroundDetectionEnabled

        // Ensure the main NfcControlActivity is always enabled
        val mainComponentName = ComponentName(context, NfcControlActivity::class.java)
        try {
            if (context.packageManager.getComponentEnabledSetting(mainComponentName) != PackageManager.COMPONENT_ENABLED_STATE_ENABLED) {
            ...
            aapsLogger.error(LTag.NFC, "Failed to enable NfcControlActivity", e)
        }

        val backgroundComponentName = ComponentName(context, NfcBackgroundDetectorActivity::class.java)
        try {
            val newState = if (backgroundEnabled) {
                PackageManager.COMPONENT_ENABLED_STATE_ENABLED
            } else {
                PackageManager.COMPONENT_ENABLED_STATE_DISABLED
            }
            val currentState = context.packageManager.getComponentEnabledSetting(backgroundComponentName)
            if (currentState != newState) {
                ...
                aapsLogger.debug(LTag.NFC, "NfcBackgroundDetectorActivity state forced to: $backgroundEnabled")
            }
        } catch (e: Exception) {
            aapsLogger.error(LTag.NFC, "Failed to update NfcBackgroundDetectorActivity state", e)
        }
    }
    ...
    override fun getPreferenceScreenContent() = PreferenceSubScreenDef(
        ...
        items = listOf(
            ...
            BooleanKey.NfcEnableBackgroundDetection,
    ...
    private suspend fun processBasal(divided: List<String>): NfcExecutionResult {
        ...
        return when {
            divided[1].equals("STOP", ignoreCase = true) || divided[1].equals("CANCEL", ignoreCase = true) -> {
                val bolusResult = commandQueue.cancelTempBasal(enforceNew = true)

                if (!bolusResult.success) aapsLogger.error(LTag.NFC, "cancelTempBasal failed: ${bolusResult.comment}")
            ...
            divided[1].endsWith("%")                                                                       -> {
                ...
                val bolusResult = commandQueue.tempBasalPercent(
                    tempBasalPct,
                    duration,
                    true,
                    profile,
                ...
                if (!bolusResult.success) aapsLogger.error(LTag.NFC, "tempBasalPercent failed: ${bolusResult.comment}")
                ...
                val bolusResult = commandQueue.tempBasalAbsolute(
                    tempBasal,
                    duration,
                    true,
                    profile,
                ...
                if (!bolusResult.success) aapsLogger.error(LTag.NFC, "tempBasalAbsolute failed: ${bolusResult.comment}")
    ...
    private suspend fun processExtended(divided: List<String>): NfcExecutionResult {
        ...
        return when {
            divided[1].equals("STOP", ignoreCase = true) || divided[1].equals("CANCEL", ignoreCase = true) -> {
                val bolusResult = commandQueue.cancelExtended()

                if (!bolusResult.success) aapsLogger.error(LTag.NFC, "cancelExtended failed: ${bolusResult.comment}")
            ...
            divided.size != 3                                                                              -> invalidFormat()
                ...
                val bolusResult = commandQueue.extendedBolus(
                    extended,
                    duration
                )
                if (!bolusResult.success) aapsLogger.error(LTag.NFC, "extendedBolus failed: ${bolusResult.comment}")
    ...
    private suspend fun processBolus(divided: List<String>): NfcExecutionResult {
        ...
        if (!bolusResult.success) {
            aapsLogger.error(LTag.NFC, "bolus failed: ${bolusResult.comment}")
        } else {
